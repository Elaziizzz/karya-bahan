...
const exportExcelRegex = /const exportExcel = async \(\) => \{[\s\S]*?saveAs\(blob, \`Laporan_PnL_\$\{activeStore\}_\$\{selectedFilter\.replace\(' ', '_'\)\}\.xlsx\`\);\s*\};/;

const match = content.match(exportExcelRegex);
if (!match) {
  console.log("exportExcel match failed");
} else {
  console.log("Found exportExcel, replacing...");
  // Use the same replacement as before
