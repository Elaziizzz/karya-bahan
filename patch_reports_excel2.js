...
// find start of exportExcel and the end (which is just before return ( )
const startIndex = content.indexOf("const exportExcel = async () => {");
if (startIndex !== -1) {
  // Find the exact closing brace of this function by looking for the return ( statement
  // actually, exportExcel ends around `saveAs(...);`
  
  const endMarker = "saveAs(blob, `Laporan_PnL_${activeStore}_${selectedFilter.replace(' ', '_')}.xlsx`);";
  ...
  if (endIndex !== -1) {
    ...
    console.log("exportExcel manually replaced.");
  } else {
    console.log("Could not find saveAs in exportExcel");
  }
}
